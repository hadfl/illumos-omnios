 ...
 * the state of the command ring. However, there is one xd_imtx per device.
...
xhci_firmware_load(xhci_t *xhcip)
{
	...
	/* XXXARM: compatible property looks wrong */
	if (!prom_fdt_is_compatible(promif_rootnode(),
	    "RaspberryPi,4,raspberrypi,4-model-b"))
		return (B_TRUE);
	...
	rv = ddi_prop_lookup_int_array(DDI_DEV_T_ANY, xhcip->xhci_dip,
	    DDI_PROP_DONTPASS, "reg", &regp, &reglen);
